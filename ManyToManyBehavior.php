 ...
 * @package kak\behaviors
 * @see This project fork https://github.com/voskobovich/ManyToManyBehavior
...
class ManyToManyBehavior extends \yii\base\Behavior
{
    ...
    public function deleteRelations($event)
    {
        ...
        foreach ($this->relations as $attributeName => $params) {
            ...
            if (!empty($relation->via) && $relation->multiple) {
                ...
                try {
                    ...
                    if($isNested && !empty($bindingKeys)) {
                        $this->insertRelationData($connection,$junctionTable,$junctionColumn, $bindingKeys, $primaryModelPk);
                    }
                    else if (!empty($bindingKeys))
                    {
                        $junctionRows = [];
                ...
                try {
                    ...
                    // Remove old relations
                    $connection->createCommand()
                        ->delete($manyTable, "{$manyTableFkColumn} = :id", [':id' => $primaryModelPk])
                        ->execute();

                    if($isNested && !empty($bindingKeys)) {

                        $this->insertRelationData($connection,$manyTable,$manyTableFkColumn, $bindingKeys, $primaryModelPk);

                    } else if(!empty($bindingKeys)) {

                        $connection->createCommand()
                            ->batchInsert($manyTable, [$manyTableFkColumn], $bindingKeys)
                            ->execute();
                    }
     ...
     * @param $connection
     * @param $junctionTable
     * @param $junctionColumn
     * @param $relatedItems
     * @param $primaryModelPk
     */
    private function insertRelationData($connection,$junctionTable,$junctionColumn, $relatedItems, $primaryModelPk)
    {
        foreach($relatedItems as &$relatedItem) {
            $relatedItem[$junctionColumn] = $primaryModelPk;
            $connection->createCommand()
                ->insert($junctionTable, $relatedItem)
                ->execute();
        }
    }
